...
def ai_hint():
    ...
    if game_type == "chemistry_generation":
        ...
        try:
            ...
            generated_compound = json.loads(raw)
...
Sometimes the level is not completable, and they will have to click restart.
...
"""
        ...
        prompt = f"""
You are an electrical engineer inside a circuitry sandbox demo for kids.
...
Explain clearly and briefly using simple words that a child could understand.
No more than three small sentences.
Focus on the state of the circuit and what the player can do to complete the level.
"""
...
@app.route("/ai_hint_test", methods=["POST"])
def ai_hint_test():
    data = request.get_json()
    print("Received:", data)
    ...
    return jsonify({
        "reply": f"Game mode received: {game_type}"
    })
